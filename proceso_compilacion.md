...
> **R:Porque incluye todas las lineas de codigo de las librerias definidas al principio.**
...
> **R:El comando no devuelve nada. Esto se debe a que el preprocesador elimina todos los comentarios del código fuente original al generar el archivo expandido**
...
> **R:gcc -E programa.c | grep "Iniciando" - $ gcc -E -DDEBUG programa.c | grep "Iniciando"    printf("[DEBUG] %s\n", ("Iniciando main"));**
...
> **R:Esas lineas comunican al compilador y al debugger la ubicación original del código que sigue a continuación.proviene de programa.c q es donde esta el #include <stdio.h>**
...
> **R:$ grep "area_circulo" programa.s .ascii "area_circulo(%.1f) = %.4f\12\0"  call    _area_circulo  .def    _area_circulo;  .scl    2;      .type   32;     .endef**
...
> **R:$ grep -A 5 "_sumar:" programa.s _sumar: LFB14: .cfi_startproc pushl   %ebp .cfi_def_cfa_offset 8 .cfi_offset 5, -8**

Explicá en términos generales qué hacen esas instrucciones
(usá los comentarios del laboratorio como guía):

> **R:Estas instruciones son la configuracion inicial, antes de iniciar el programa se asegura su lugar en memoria, y su vuelta al main una vez que termine su tarea.**
...
> **R:Porque area_circula si esta definida en matematica.o mientras que en programa.o esta haciendo refenrencia a la definicion en matematica.o**
...
> **R: Quedan porque son funciones de la biblioteca dinámica del sistema. Los resuelve el cargador dinamico cuando el programa se ejecute.**
...
> **R:La diferencia es que una macro funcion se ejecuta automaticamente y mientras que una funcion real es un bloque de codigo independiente con su propia direccion de memoria, que cuando se ejecuta o se llama el programa saltaa esebloque ejecuta las instrucciones y vuelve con el resultado. La Macro desaparece en el preprocesamiento y la real en el enlazado. La funcion real tiene verificacion de tipos, ya que el compilador chequea que los argumentos coincidan con lo declarado antes de compilar.**
...
> **R:Que las del simbolo T son acciones o funciones mientras que las de D son datos.T vive en la seccion de .text mientras que D vive en la seccion del archivo .data**
...
> **R:R: ntdll.dll => /c/Windows/SYSTEM32/ntdll.dll (0x7ff96f470000) ntdll.dll => /c/Windows/SysWOW64/ntdll.dll (0x772e0000) wow64.dll => /c/Windows/System32/wow64.dll (0x7ff96f390000)        wow64win.dll => /c/Windows/System32/wow64win.dll (0x7ff96db10000)**

¿Por qué `libc` no hubo que especificarla explícitamente al enlazar con `gcc`?

> **R:No hizo falta porque gcc esta diseñado para hacerlo automaticamente.**
